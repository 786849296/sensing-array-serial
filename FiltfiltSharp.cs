...
namespace FiltfiltSharp
{
	public class FiltfiltHelper
	{
		public static List<double> DoFiltfilt(List<double> b, List<double> a, List<double> x)
		{
			...
			double[][] zZi = MatrixMultiplication(MatrixInversion(sp), Calc(Segment([.. b], 1, nFilt - 1), b.ToArray()[0], Segment([.. a], 1, nFilt - 1)));
		...
		private static void AddIndexRange(List<int> indices, int beg, int end, int inc)
		{
			for (int i = beg; i <= end; i += inc)
				indices.Add(i);
		}

		private static void AddIndexConst(List<int> indices, int value, int numel)
		{
			while (numel-- != 0)
				indices.Add(value);
		}
		...
		{
			List<double> resultArrayList = new(idxEnd - idxStart + 1);
